<?php
    ...
    if (strlen($fileName) === 0){
        ...
        if ($conn->query($updateDataQuery) === TRUE) {
            header("Location: facultyProfile.php?userName=".$username."&uniLogo=".$uni_logo."&depttLogo=".$deptt_logo);
        } else {
        ...
        if (move_uploaded_file($_FILES["picture"]["tmp_name"], $targetPath)) {
            ...
            if ($conn->query($updateDataQuery) === TRUE) {
                header("Location: faculty.php?userName=".$username."&uniLogo=".$uni_logo."&depttLogo=".$deptt_logo);
            } else {
                echo "Error saving file path to database: " . $conn->error;
            }
        } else {
