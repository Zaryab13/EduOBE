document.addEventListener('DOMContentLoaded', () => {
  const addQuizAssingmentMarksButton = document.getElementById(
    'addQuizAssingmentMarksBtn'
  );
  const addQuizAssingmentMarksModalTitle = document.getElementById(
    'addQuizAssingmentMarksModalLabel'
  );
  const QuizAssingmentMarksModalAddUpdateBtn = document.getElementById(
    'QuizAssingmentMarksModalAddUpdateBtn'
  );

  addQuizAssingmentMarksButton.addEventListener('click', () => {
    addQuizAssingmentMarksModalTitle.innerHTML = 'Add Marks';
    QuizAssingmentMarksModalAddUpdateBtn.innerHTML = 'Add';
  });

  const tableQuizAssingmentMarks = $(
    '#QuizAssingmentMarks-dataTable'
  ).DataTable({
    paging: true,
  ...
  // Add event listener for edit button in QuizAssingmentMarks DataTable
  $('#QuizAssingmentMarks-dataTable tbody').on(
    'click',
    'button.edit-button',
    function () {
      let rowData = tableQuizAssingmentMarks.row($(this).closest('tr')).data();
      console.log('QuizAssingmentMarks-Table : ');
      console.log(rowData);

      // updating inner HTML content of modal title and action button.
      addQuizAssingmentMarksModalTitle.innerHTML = 'Update Marks';
      QuizAssingmentMarksModalAddUpdateBtn.innerHTML = 'Update';

      $('#addUpdateQuizAssingmentMarksModal').modal('show');
  ...
  $('#QuizAssingmentMarks-dataTable tbody').on(
    ...
    function () {
      deleteRowData = tableQuizAssingmentMarks
        .row($(this).closest('tr'))
        .data();
      console.log('QuizAssingmentMarks-Table : ');
      console.log(deleteRowData);

      // Open deleteQuizAssingmentMarksConfirmationModal
      $('#deleteQuizAssingmentMarksConfirmationModal').modal('show');
    }
  );

  $('#confirmDeleteQuizAssingmentMarksBtn').on('click', () => {
  ...
  $('#deleteQuizAssingmentMarksConfirmationModal').on(
    'hidden.bs.modal',
    function () {
  ...
  QuizAssingmentMarksData.forEach((object) => {
    tableQuizAssingmentMarks.row.add([
      object.id,
      object.code,
      object.name,
      object.type,
