document.addEventListener('DOMContentLoaded', () => {
  const addMidMarksButton = document.getElementById('addMidMarksBtn');
  const addMidMarksModalTitle = document.getElementById(
    'addMidMarksModalLabel'
  );
  const MidMarksModalAddUpdateBtn = document.getElementById(
    'MidMarksModalAddUpdateBtn'
  );

  addMidMarksButton.addEventListener('click', () => {
    addMidMarksModalTitle.innerHTML = 'Add Marks';
    MidMarksModalAddUpdateBtn.innerHTML = 'Add';
  });

  const tableMidMarks = $('#MidMarks-dataTable').DataTable({
    paging: true,
  ...
  // Add event listener for edit button in MidMarks DataTable
  $('#MidMarks-dataTable tbody').on('click', 'button.edit-button', function () {
    let rowData = tableMidMarks.row($(this).closest('tr')).data();
    console.log('MidMarks-Table : ');
    console.log(rowData);

    // updating inner HTML content of modal title and action button.
    addMidMarksModalTitle.innerHTML = 'Update Marks';
    MidMarksModalAddUpdateBtn.innerHTML = 'Update';

    $('#addUpdateMidMarksModal').modal('show');
  ...
  let deleteRowData;
  $('#MidMarks-dataTable tbody').on(
    'click',
    'button.delete-button',
    function () {
      deleteRowData = tableMidMarks.row($(this).closest('tr')).data();
      console.log('MidMarks-Table : ');
      console.log(deleteRowData);

      // Open deleteMidMarksConfirmationModal
      $('#deleteMidMarksConfirmationModal').modal('show');
    }
  );

  $('#confirmDeleteMidMarksBtn').on('click', () => {
  ...
  $('#deleteMidMarksConfirmationModal').on('hidden.bs.modal', function () {
  ...
  MidMarksData.forEach((object) => {
    tableMidMarks.row.add([
      object.id,
      object.code,
      object.name,
      object.type,
