document.addEventListener('DOMContentLoaded', () => {
    ...
    const tableCQI = $('#CQI-dataTable').DataTable({
      paging: true,
    ...
    // Add event listener for edit button in CQI DataTable
    $('#CQI-dataTable tbody').on('click', 'button.edit-button', function () {
      let rowData = tableCQI.row($(this).closest('tr')).data();
      console.log('CQI-Table : ');
      console.log(rowData);
  
      // updating inner HTML content of modal title and action button.
      addCQIModalTitle.innerHTML = 'Update CQI';
      CQImodalAddUpdateBtn.innerHTML = 'Update';
  
      // Open addCQIModal
      $('#addCQIModal').modal('show');
  
      // Use the rowData to populate the fields in the modal
      $('#addCQIModal input[name="CQI-code"]').val(rowData.Code);
      $('#addCQIModal input[name="CQI-name"]').val(rowData.Name);
