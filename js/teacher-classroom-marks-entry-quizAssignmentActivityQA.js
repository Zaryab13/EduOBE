document.addEventListener('DOMContentLoaded', () => {
    const addMarksEntryQAbutton = document.getElementById('addMarksEntryQABtn');
    const addMarksEntryQAModalTitle = document.getElementById('addMarksEntryQAModalLabel');
    const MarksEntrymodalQAAddUpdateBtn = document.getElementById('MarksEntry-modalAddUpdateBtn');
  
    addMarksEntryQAbutton.addEventListener('click', () => {
      addMarksEntryQAModalTitle.innerHTML = 'Add Marks';
      MarksEntrymodalAddUpdateBtn.innerHTML = 'Add';
    });
  
    const tableMarksEntry = $('#MarksEntryQA-dataTable').DataTable({
      paging: true,
    ...
    $('#MarksEntry-dataTable tbody').on('click', 'button.edit-button', function () {
      ...
      addMarksEntryQAModalTitle.innerHTML = 'Update Marks';
      MarksEntrymodalAddUpdateBtn.innerHTML = 'Update';
  
      // Open addMarksEntryQAModal
      $('#addMarksEntryQAModal').modal('show');
  
      // Use the rowData to populate the fields in the modal
      $('#addMarksEntryQAModal input[name="MarksEntry-code"]').val(rowData.Code);
      $('#addMarksEntryQAModal input[name="MarksEntry-name"]').val(rowData.Name);
